...
namespace ApiDotflix.Controllers
{
    ...
    {
        private readonly IBaseRepository<RoadMap> _baseRepository;

        public RoadMapController(IBaseRepository<RoadMap> baseRepository)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<RoadMap>> GetAllAsync()
        {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<RoadMap>> GetByIdDirector(int id)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync(RoadMap entity)
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(RoadMap entity)
        {
            if (!ModelState.IsValid) return BadRequest(new ValidationProblemDetails(ModelState));
